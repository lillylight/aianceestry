...
export async function downloadAnalysisAsPDF(
  ...
  doc.setFontSize(32);
  doc.setTextColor('#23252b');
  doc.text('Ancestry Analysis Report', pageWidth / 2, 80, { align: 'center' });
  
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(16);
  doc.setTextColor('#555');
  doc.text('Generated with AI Ancestry', pageWidth / 2, 115, { align: 'center' });
  
  doc.setFontSize(12);
  doc.setTextColor('#888');
  doc.text('Date: ' + new Date().toLocaleDateString(), pageWidth / 2, 145, { align: 'center' });
  
  // Add user wallet address if available
  let yPosition = 175; // Starting Y position after basic info
  if (typeof window !== 'undefined' && (window as any).aiAncestryUserName) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(13);
    doc.setTextColor('#2f80ed');
    doc.text('Generated by: ' + (window as any).aiAncestryUserName, pageWidth / 2, yPosition, { align: 'center' });
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(12);
    doc.setTextColor('#888');
    yPosition += 30; // Add space after wallet address
  }
  
  // Add user photo if available, now positioned below wallet address
  if (userPhotoUrl) {
    try {
      ...
      // Add image to PDF
      doc.addImage(img, 'PNG', pageWidth / 2 - imgWidth / 2, yPosition, imgWidth, imgHeight);
      
      // Add photo caption
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      doc.setTextColor('#23252b');
      doc.text('Your Photo', pageWidth / 2, yPosition + imgHeight + 10, { align: 'center' });
      
      // Update Y position for content after photo
      yPosition += imgHeight + 40;
    } catch (error) {
      console.error('Error adding photo to PDF:', error);
      yPosition += 40; // Add some space even if photo fails to load
    }
  } else {
    yPosition += 40; // Add space even if no photo
  }
